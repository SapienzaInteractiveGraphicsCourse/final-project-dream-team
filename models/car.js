...
class Car {
  constructor(character, speedCar, initialPosition, directionCar) {
    this.characterReference = character; //needed to check if car runs over the character
  ...
  goForward(referencePositionCharacter){ //x++ right with respect of the camera, y++ height to the high, z++ front closer to the camera (x, y, z)
    ...
    if( (Math.abs(referencePosition.x - referencePositionCharacter.x) <= this.sideX + this.characterReference.sideX) &&
        (Math.abs(referencePosition.y - referencePositionCharacter.y) <= this.sideY + this.characterReference.sideY) &&
        (Math.abs(referencePosition.z - referencePositionCharacter.z) <= this.sideZ + this.characterReference.sideZ) ){

          var newX = this.direction*(referencePosition.x - referencePositionCharacter.x);
          var newZ = this.direction*(referencePositionCharacter.z - referencePosition.z);
